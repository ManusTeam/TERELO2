<?php
	//
	// TERELO2 Php proxy "szerver"
	// Mielött felrakod: kibaszhatnak a host oldalról ezért.
	// Néhány host nem szereti, ha proxynak használod a szervereiket.
	// Csak mondom, de biztos találsz jót.
	//
$whitelistPatterns = [
...
$forceCORS = false;
//helyi weboldalt elengedi
$disallowLocal = true;
//false = elküldi a felhasználó ipt szerver ip helyett
$anonymize = true;
$startURL = "";
...
foreach($requiredExtensions as $requiredExtension) {
  if (!extension_loaded($requiredExtension)) {
    die("PHP extension kell: \"" . $requiredExtension . "\" Rakd Fel!");
  }
}
...
function makeRequest($url) {
  ...
  if (empty($user_agent)) {
    $user_agent = "Mozilla/5.0 (compatible; TERELO2)";
  }
  $ch = curl_init();
...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>TERELO2</title></head><body><h1>TERELO2 Proxy</h1>Direktbe letudsz kérdezni: <a href=\"" . PROXY_PREFIX . $landingExampleURL . "\">" . PROXY_PREFIX . $landingExampleURL . "</a><br /><br />Vagy ide beírod a linket:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"Mehet a proxy!\" /></form></body></html>");
    } else {
      $url = $startURL;
    }
